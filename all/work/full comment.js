if (window.location.href.includes("cases.connect")) {
    (() => {
        // Singleton check: prevents script from running twice
        ...
        /** * Safely finds a DOM element and clicks it if it exists. 
         */
        ...
        /** * Adds a number of business days to a start date, skipping weekends 
         * and provided holidays.
         */
        ...
        /** * Returns a Promise that resolves when a specific DOM element appears 
         * and is visible (has an offsetParent). Times out after a set duration.
         */
        ...
        /** * Waits for an element, optionally traverses to a sibling ('steps'), 
         * clicks it, and returns the clicked element.
         */
        ...
        /** * Sets up a MutationObserver to watch for a specific node being added to the DOM,
         * triggers the callback, and keeps observing.
         */
        ...
        /** * Reusable component for creating consistent UI buttons 
         * with hover effects and styles.
         */
        class Button {
            constructor({
                text = "",
            ...
            // Appends the button to a parent container
            ...
            // Updates button background color dynamically
            setColor = (color) => {
                this.element.style.backgroundColor = color;
            }

            // Allows nesting elements inside the button (e.g., input fields)
            appendChild = (childElement) => {
                this.element.appendChild(childElement);
            }
        }

        /** * Logic for the "Auto Click" toggle button.
         * Periodically clicks a 'connect' button and then a 'remove' button.
         */
        const autoBtn = {
            timerId: null,
            on: false,
            ...
            // Updates the button text (ON/OFF) and color
            updateView() {
        ...
        /** * Logic for the "Check Follow-up" button.
         * Navigates to the home screen and opens the follow-up list.
         * Also monitors the list for a "badge" count.
         */
        const checkBtn = {
            // Sequence: Click Home -> Wait -> Click Follow-up item
            async handleClick() {
                click(config.selectors.homeButton);
                await waitClick(config.selectors.flupItem, 0);
            },

            // Shows/Hides a red badge based on the 'data-attr' count
            updateBadge(el) {
            ...
            // Creates the button and starts an observer on the target element
            create(parent) {
                const htmlContent = `
        ...
        /** * Logic for the "Set Follow Up" button.
         * Calculates future dates based on user input and automates the calendar selection.
         */
        const followUpBtn = {
            inputEl: null,

            // Complex Logic:
            // 1. Checks if calendar is open; opens if not.
            // 2. Calculates N working days from now.
            // 3. Calculates total calendar days difference.
            // 4. Clicks the "Next Day" arrow that many times.
            ...
            // Creates the button and the embedded text input field
            create(parent) {
                const btn = new Button({
                    text: "FL Up:",
        ...
        /** * Plays an audio alert when the phone dialog appears.
         */
        ...
        /** * Injects CSS to style the input field inside the follow-up button.
         */
        ...
        /** * Creates the fixed container panel in the bottom-left corner.
         */
        const createPanel = () => {
        ...
        /** * Main entry point: sets up observers, styles, and creates the UI panel.
         */
        const init = () => {
    ...
    (() => {
        ...
        class AgentDashboard {
            ...
            constructor() {
                ...
                // 3. Initialize Security Policy (TrustedTypes)
                this.#initTrustedTypes();

                // 4. Normalize Icon URLs
            ...
            /** * Configures TrustedTypes policy to allow safe HTML injection 
             * (Required in some secured corporate environments).
             */
            #initTrustedTypes = () => {
                if (window.trustedTypes && window.trustedTypes.createPolicy) {
                    ...
                    // Fallback for browsers without TrustedTypes support
                    this.#trustedPolicy = {
                        createHTML: (s) => s
                    };
                }
            ...
            /** * Updates relative icon paths to absolute URLs.
             */
            #normalizeIconUrls = () => {
                const {
                    icons,
            ...
            /** * Extracts the logged-in user's ID from their profile image URL
             * so they can be pinned to the top of the list.
             */
            #getCurrentUserLdap = () => {
            ...
            /** * Creates the hidden HTML container for the dashboard overlay.
             * Adds an event listener for the "Close" button.
             */
            #createOverlay = () => {
            ...
            /** * Observes the target HTML table for any changes (data updates)
             * and triggers a re-render.
             */
            #initObserver = () => {
                // Note: .bind(this) is no longer needed because #render is now an arrow function field
            ...
            /** * Hides the UI and disconnects observers (cleanup).
             */
            #destroy = () => {
            ...
            /** * Iterates over DOM rows, extracting raw text for Name, Status, Time, etc.,
            ...
            #scrapeTableData = () => {
                ...
                return Array.from(rows, (tr) => {
                    ...
                    // Extract 'Phone Status' (e.g., 'Not Ready', 'Busy')
                    ...
                    return {
                        ...
                        auxCode: cells[3].innerText, // e.g., 'Active', 'Lunch'
             ...
             * If an agent is "Active" but their phone is "Busy", 
             * reclassify them as "Break" (Unavailable).
             * Sets CSS classes for color coding.
             */
            #normalizeAgentStatus = (agent) => {
                let displayStatus = agent.auxCode;
                let statusKey = agent.auxCode.toLowerCase().replace(/\s+/g, "-");

                // Logic Override: If Active but Phone is Busy, count as 'Break' (unavailable)
            ...
            /** * Sorting Algorithm:
             * 1. Current User (Top)
             * 2. Status Priority (Active > Phone > Lunch, etc.)
             * 3. Duration (Longest duration first)
             */
            #sortAgents = (agents) => {
            ...
            /** * Generates the HTML string for a single agent row.
             */
            ...
            /** * Returns the HTML image tag for the specific status icon.
             */
            ...
            /** * Helper: Converts strings like "1h 30m" into total seconds for sorting.
             */
            ...
            /** * Injects the CSS styles for the dashboard overlay into the head.
             */
            #injectStyles = () => {
    ...
    (() => {
        ...
        /** * Enhances a DOM element with "Click to Copy" functionality.
         * Visual feedback: Changes background color briefly on success.
         */
        const initCopy = ({
            el,
            text,
        ...
        /** * Decodes the specific data array structure.
         * Determines if the row is GA4 or Ads Conversion and extracts the Label.
         */
        ...
        /** * Creates a floating box at the bottom left displaying the "AW-ID".
         * Clicking the box copies the ID.
         */
        const showAwId = (id) => {
        ...
        /** * Iterates through the table rows on the page.
         * 1. Filters out non-web conversion sources.
         * 2. Matches row name to the data map.
         * 3. Updates the cell HTML with the real Label.
         * 4. Applies color coding (Yellow for GA4, Green for Ads).
         */
         ...
         * 1. Extracts the global 'SHARED_ALL_ENABLED_CONVERSIONS' string.
         * 2. Regex matches the Account ID (AW-ID).
         * 3. Expands table rows (clicks "expand more").
         * 4. Parses the JSON data and builds a Map.
         * 5. Calls processRows.
         */
        const run = () => {
        ...
        /** * Polling mechanism:
         * Checks if the global variable `conversions_data` is loaded.
         * Retries up to `maxTries` times before giving up.
         */
        const poll = () => {
            if (
