(function () {
	...
	class AdWords {
		...
		/**
		 * Lấy Conversion ID gốc từ dòng đã chọn (Row Selected)
		 */
		static getSelectedConversionIds() {
			...
			const selectedIds = rows
				...
				.map((row) => {
					...
					// Trích xuất ctId từ link href nếu có (Ví dụ: ctId=7702302629)
					const hrefCtId = link?.href?.match(/ctId=(\d+)/)?.[1];

					// Ưu tiên 1: Lấy từ data-original-id đã lưu trước đó
					// Ưu tiên 2: Lấy từ ctId trong link
					// Ưu tiên 3: Lấy từ text thuần
					const rawId =
				...
				setTimeout(() => {
					Utils.$$(".conversion-name-cell .internal").forEach(
						(cell) => {
							...
							// LƯU LẠI NUMERIC ID GỐC VÀO DATASET TRƯỚC KHI THAY THẾ TEXT
							...
							if (type && convId) {
								cell.textContent = convId; // Thay thế giao diện bằng Conversion Label
