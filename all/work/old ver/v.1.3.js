			...
			const config = {
				...
				icons: {
					video: {
						...
						animation: "breathe", // New: pulses scale and opacity like recording
					},
					"coffee-break": {
						src: `${iconBase}/16108/16108931.png`,
						animation: "rock", // New: gentle side-to-side relaxing rock
					},
					"lunch-break": {
						src: `${iconBase}/1182/1182132.png`,
						animation: "bounce-y", // New: small vertical jump
					},
					phone: {
						src: `${iconBase}/13332/13332839.png`,
						animation: "ring", // New: vigorous shake like a ringing phone
					},
					email: {
						src: `${iconBase}/7487/7487055.png`,
						animation: "fly", // New: diagonal movement like sending mail
					},
					break: {
						src: `${iconBase}/5140/5140652.png`,
						animation: "fade-pulse", // New: slow fading indicating standby
					},
						...
						.map((group) => {
							let rowsHtml = group.rows
								.map((agent) => {
									...
									// Only display non-icons if they are strictly in the active status
									let isActive = agent.statusKey === "active";

									// Create background icon HTML
									let nonPhoneImg =
