    ...
    (() => {
        ...
        let utils = {
            ...
            addStyle(id, cssText) {
                ...
                styleEl.id = id;
                let policy = window.trustedTypes?.createPolicy("default", { createHTML: (e) => e }) ?? { createHTML: (e) => e };
                styleEl.textContent = policy.createHTML(cssText);
                document.head.appendChild(styleEl);
            },
            createEl(tag, { parent, onClick, style, className, id, ...props } = {}) {
            ...
            setupCopy(element, text, successMsg = "Copied!") {
                ...
                element.addEventListener("click", async (e) => {
                    try {
                        ...
                        element.dataset.origText || (element.dataset.origText = element.innerText);
        ...
        let features = {
            casemon() {
                ...
                let config = {
                    ...
                    statusConfig: {
                        active: { color: "#10B981", track: "#D1FAE5", maxSecs: 3600 },
                        phone: { color: "#EF4444", track: "#FFE4E6", maxSecs: 2700 },
                        video: { color: "#8B5CF6", track: "#F3E8FF", maxSecs: 2700 },
                        email: { color: "#0EA5E9", track: "#E0F2FE", maxSecs: 900 },
                        "coffee-break": { color: "#F59E0B", track: "#FFEDD5", maxSecs: 900 },
                        "lunch-break": { color: "#EAB308", track: "#FEF9C3", maxSecs: 3600 },
                        break: { color: "#6B7280", track: "#F3F4F6", maxSecs: 900 },
                        default: { color: "#9CA3AF", track: "#F3F4F6", maxSecs: 2700 },
                    },
                    icons: {
                        video: { src: `${iconBase}/9571/9571236.png`, animation: "wiggle" },
                        "coffee-break": { src: `${iconBase}/16108/16108931.png`, animation: "wiggle" },
                        "lunch-break": { src: `${iconBase}/1182/1182132.png`, animation: "pulse" },
                        phone: { src: `${iconBase}/13332/13332839.png`, animation: "wiggle" },
                        email: { src: `${iconBase}/7487/7487055.png`, animation: "slide" },
                        break: { src: `${iconBase}/5140/5140652.png`, animation: "wiggle" },
                        close: `${iconBase}/9403/9403346.png`,
                    },
                    priorities: { active: 1, video: 2, phone: 2.5, "lunch-break": 3, email: 4, "coffee-break": 5, break: 6, default: 99 },
                };
                ...
                let currentUserName = utils.$("[alt='profile photo']")?.src?.match(/photos\/([^/?]+)/)?.[1] ?? "Unknown";
              ...
              `
                ...
                let uiContainer =
                    ...
                    utils.createEl("div", { id: config.uiId, parent: document.body });
                let lastHtml = "";
                ...
                let parseTime = (timeStr) =>
                    (timeStr.match(timeRegex) || []).reduce(
                        (acc, curr) =>
                            acc + parseInt(curr, 10) * ({ h: 3600, m: 60, s: 1 }[curr.slice(-1)] || 0),
                        0
                    );

                let updateDashboard = () => {
                    try {
                        let parsedAgents = Array.from(targetContainer.querySelectorAll("tbody tr"))
                            .map((row) => {
                                let cells = row.querySelectorAll("td");
                                if (!cells || cells.length < 10) return null;

                                let rawStatus1 = (cells[5].innerText.match(textRegex)?.[0] || "").trim().toLowerCase().replace(/\s+/g, "-");
                                let rawStatus2 = (cells[8].innerText.match(textRegex)?.[0] || "").trim().toLowerCase().replace(/\s+/g, "-");
                            ...
                            .sort((a, b) => {
                                ...
                                let aPriority = config.priorities[a.statusKey] ?? config.priorities.default;
                                let bPriority = config.priorities[b.statusKey] ?? config.priorities.default;

                                return aPriority !== bPriority ? aPriority - bPriority : b.durationSeconds - a.durationSeconds;
                            });

                        let activeCount = parsedAgents.filter((agent) => agent.statusKey === "active").length;
                        let callCount = parsedAgents.filter((agent) => ["phone", "video"].includes(agent.statusKey)).length;
                        let breakCount = parsedAgents.filter((agent) => !["phone", "video", "active"].includes(agent.statusKey)).length;
                        let totalCount = parsedAgents.length;

                        let isLowAvailability = (totalCount > 0 ? activeCount / totalCount : 0) < 0.2 && totalCount > 0;
                        let groups = [];
                            ...
                            .map((group) => {
                                let rowsHtml = group.rows
                                    .map((agent) => {
                                        ...
                                        let stConf = config.statusConfig[agent.statusKey] || config.statusConfig.default;
                                        let maxSecs = stConf.maxSecs || 2700;
                                        let progressPct = Math.min((agent.durationSeconds / maxSecs) * 100, 100).toFixed(1);
                                        let isOverTime = agent.durationSeconds >= maxSecs;
                ...
                let observer = new MutationObserver(utils.debounce(updateDashboard, 150));
                observer.observe(targetContainer, { attributes: true, childList: true, subtree: true, characterData: true });
              ...
              `
                ...
                utils.createEl("button", {
                    ...
                    async onClick() {
                        ...
                        try {
                            let popup = await utils.waitForElement(".li-popup_lstcasefl");
                            popup?.click();
                        } catch (err) {
                    ...
                    async onClick(e) {
                        if (e.target.id !== "flup-days-input") {
                            try {
                                ...
                                let apptEl = utils.$('[data-infocase="appointment_time"]');
                                if (apptEl && !apptEl.dataset.valchoice) {
                                    apptEl.click();
                                    await utils.sleep(150);
                                    let todayEl = await utils.waitForElement(".datepicker-grid .today");
                                    if (todayEl) todayEl.click();
                                    await utils.sleep(200);
                                }

                                let daysOffset = parseInt(utils.$("#flup-days-input").value, 10) || 0;
                                let flupTimeEl = utils.$('[data-infocase="follow_up_time"]');
                                ...
                                if (daysOffset > 0) {
                                    ...
                                    let diffDays = Math.round((targetDate - new Date()) / 86400000);
                                    ...
                                    let finishEl = await utils.waitForElement('[data-thischoice="Finish"]');
                                    if (finishEl) {
                                        finishEl.click();
                                        await utils.sleep(200);
                                    }
                                ...
                                let typeEl = await utils.waitForElement("[data-type=follow_up_time]");
                ...
                let insertSignature = () => {
                    ...
                    if (!sel.rangeCount) {
                        alert("Please click inside the email body to place your cursor first.");
                        return;
                    }
                    let node = sel.getRangeAt(0).startContainer.parentNode;
                    if (!node || !node.closest("[contenteditable]")) {
                        alert("Please place your cursor inside the text area where you want the signature.");
                        return;
                    }

                    document.querySelectorAll(".aw-sig-table").forEach((el) => el.remove());
                    ...
                    document.execCommand(
                        ...
                        ((html) => {
                            if (window.trustedTypes && window.trustedTypes.createPolicy) {
                                let policy = trustedTypes.createPolicy("sig-inject", { createHTML: (str) => str });
              ...
              `
                ...
                let processAdwordsData = (rawData) => {
                    ...
                    if (uniqueIds.length > 0) {
                        let container = utils.$("#gpt-aw-container") || utils.createEl("div", { id: "gpt-aw-container", parent: document.body });
                        container.innerHTML = "";

                        uniqueIds.forEach((idStr) => {
                            let badge = utils.createEl("div", { className: "gpt-aw-badge", parent: container });
                            badge.textContent = `AW-${idStr}`;
                            utils.setupCopy(badge, idStr, "Copied!");
                        });
                    }

                    document.querySelectorAll(".expand-more").forEach((el) => el.click());

                    try {
                        let parsed = JSON.parse(rawData);
                        if (!parsed || !parsed[1]) return;

                        let dataMap = new Map(parsed[1].map((item) => [item[1], item]));

                        setTimeout(() => {
                            document.querySelectorAll(".conversion-name-cell .internal").forEach((cell) => {
                                let row = cell.closest(".particle-table-row");
                                if (row && !row.querySelector('[essfield="aggregated_conversion_source"]')?.innerText?.toLowerCase().includes("web")) {
                                    return row.remove();
                                }

                                let mappedData = dataMap.get(cell.innerText);
                                ...
                                if (mappedData[11] === 1) {
                                    type = "aw-ads";
                                    convId = mappedData[64]?.[2]?.[4]?.split("'")?.[7]?.split("/")?.[1];
                                } else if (mappedData[11] === 32) {
                                    type = "aw-ga4";
                                    convId = mappedData[64]?.[1]?.[4]?.split("'")?.[3];
                                }
                            ...
                            document.querySelectorAll("category-conversions-container-view, conversion-goal-card").forEach((card) => {
                ...
                let initAdwords = (attempts = 0) => {
                    let data = window.conversions_data?.SHARED_ALL_ENABLED_CONVERSIONS;
                    if (data) return processAdwordsData(data);
                    if (attempts < 5) setTimeout(() => initAdwords(attempts + 1), 600);
                };

                if (["complete", "interactive"].includes(document.readyState)) {
                    initAdwords();
                } else {
                    window.addEventListener("DOMContentLoaded", () => initAdwords());
                }
            },
        };
