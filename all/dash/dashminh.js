...
const styleSheet = () => {
  var css = `
    #agent_ui { position: fixed; height: 100%; width: 100%; top: 0; left: 0; background-color: rgba(0,0,0,0.1); backdrop-filter: blur(4px); z-index: 999; display: flex; justify-content: flex-start; align-items: center; padding: 20px; font-family: Noto Serif; }
...
const createAgentRowHtml = (agent) => {
  ...
  // ? [agent.lastChange, agent.timeSpent]
  // : [agent.timeSpent, agent.lastChange];
